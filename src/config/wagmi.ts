...
function metaMaskFlaskConnector() {
  return createConnector((config) => ({
    ...
    // @ts-ignore — wagmi connector generic type mismatch, runtime is correct
    async connect() {
      const mm = await getMetaMaskProvider()
      if (!mm) throw new Error('MetaMask not found. Please install MetaMask.')
    ...
    async getAccounts() {
      const mm = await getMetaMaskProvider()
      if (!mm) return []
      return mm.provider.request({ method: 'eth_accounts' }) as Promise<`0x${string}`[]>
    },

    async getChainId() {
      const mm = await getMetaMaskProvider()
      if (!mm) return mainnet.id
      const chainId = await mm.provider.request({ method: 'eth_chainId' })
...
export function isMetaMaskAvailable(): boolean {
  ...
  if (win.ethereum?.providers?.some((p: any) => p.isMetaMask)) return true
  return false
}
