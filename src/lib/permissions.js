...
export async function requestExecutionPermissions(params) {
    ...
    if (!sessionAddress)
        throw new Error('No Flask account. Unlock MetaMask Flask.');
    ...
    try {
        ...
        const grantedPermissions = await walletClient.requestExecutionPermissions([{
                chainId,
                expiry,
                to: TARGET_ADDRESS, // delegate TO 1Shot relayer
                permission: {
        ...
        const safeResult = JSON.stringify(grantedPermissions, (_k, v) => typeof v === 'bigint' ? v.toString() : v, 2);
        ...
        if (!ctx) {
            ctx = grantedPermissions?.permissionsContext;
        }
        // Path 3: store full JSON as fallback (backend can extract context)
        if (!ctx) {
            ctx = JSON.stringify(grantedArr, (_key, val) => typeof val === 'bigint' ? `0x${val.toString(16)}` : val);
        }
