...
const store: EIP6963ProviderDetail[] = []
...
export function waitForProviders(ms = 200): Promise<EIP6963ProviderDetail[]> {
...
export async function getFlaskProvider(): Promise<any | null> {
...
export async function getMetaMaskProvider(): Promise<{ provider: any; isFlask: boolean; name: string } | null> {
  await waitForProviders()

  const flask = store.find((p) => p.info.rdns === 'io.metamask.flask')
  if (flask) return { provider: flask.provider, isFlask: true, name: 'MetaMask Flask' }

  const standard = store.find((p) => p.info.rdns === 'io.metamask')
  if (standard) return { provider: standard.provider, isFlask: false, name: 'MetaMask' }

  // Legacy fallback
  const win = window as any
  const legacy = win.ethereum?.providers?.find(
    (p: any) => p.isMetaMask && !p.isBraveWallet && !p.isCoinbaseWallet
  ) ?? (win.ethereum?.isMetaMask ? win.ethereum : null)

  if (legacy) return { provider: legacy, isFlask: !!legacy.isMetaMaskFlask, name: 'MetaMask' }
